...
AI 能力与现有下载能力隔离在 `/api/v1/ai` 路由下。它优先读取公开视频的已有字幕；如果平台没有可下载的字幕轨道，会临时下载公开视频音频并使用本地 Whisper 生成转录，完成后删除临时音频。它不保存账号或长期历史。
...
## 界面与流式一致性修订（2026-09-20）

- 下载信息与 AI 学习工作区合并为桌面端 3:7 的同屏双栏布局；窄屏时改为单列，避免横向滚动。
- AI 工作区保留“视频大纲、字幕、思维导图、AI 问答”四个入口；首选字幕轨道会自动选中并触发总结，不再暴露字幕语言选择器。字幕转录与 SRT/TXT 导出功能仍可使用。
- 视频大纲的流式 Markdown 是唯一展示来源：SSE 产生的 `stream_text` 会随任务响应一同保留，完成后不再用独立的结构化 `outline` 覆盖，因此实时内容与最终内容完全一致。
- “重新生成”按钮在字幕加载或生成期间禁用，并提供明确的加载与错误状态；Nginx 已关闭 AI SSE 的缓冲与缓存，防止流式内容被聚合到请求结束后才显示。
- 视频格式仍由后端按 1080P、720P、480P、360P 去重筛选；前端以“选择视频清晰度”展示为 `1080p 高清`、`720p 准高清`、`480p 标清`、`360p 流畅`，每行保留一个白底文字卡片。
