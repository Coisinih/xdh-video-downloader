...
def test_to_formats_exposes_only_standard_resolutions_without_duplicates():
    payload = {
        "formats": [
            ...
            {"format_id": "480", "height": 480, "width": 854, "ext": "mp4", "vcodec": "avc1", "acodec": "aac", "tbr": 850, "url": "https://example.com/480"},
            {"format_id": "720", "height": 720, "width": 1280, "ext": "mp4", "vcodec": "avc1", "acodec": "aac", "tbr": 1200, "url": "https://example.com/720"},
            {"format_id": "1080", "height": 1080, "width": 1920, "ext": "mp4", "vcodec": "avc1", "acodec": "aac", "tbr": 2500, "url": "https://example.com/1080"},
    ...
    assert [item.label for item in formats] == ["1440P", "1080P", "720P", "480P", "360P"]
    assert [item.id for item in formats] == ["1440", "1080", "720", "480", "360-mp4"]
    assert set(sources) == {"1080", "720", "480", "360-mp4"}


def test_to_formats_routes_video_only_adaptive_streams_to_server_merging():
    formats, sources = to_formats({
        "formats": [
            {"format_id": "1080-video", "height": 1080, "width": 1920, "ext": "mp4", "vcodec": "avc1", "acodec": "none", "url": "https://example.com/video"},
        ],
    })
...
def test_youtube_command_enables_official_ejs_component():
    command = ytdlp.ytdlp_command("--dump-single-json", "https://www.youtube.com/watch?v=example", youtube=True)

    assert command[3:7] == ["--js-runtimes", "node", "--remote-components", "ejs:npm"]
...
async def test_inspect_extracts_optional_video_metadata(monkeypatch):
    payload = {
        ...
        "view_count": "6789",
        "formats": [
            {"format_id": "1080", "height": 1080, "width": 1920, "ext": "mp4", "vcodec": "avc1", "acodec": "aac", "url": "https://example.com/video.mp4"},
        ],
    }
...
async def test_inspect_uses_bilibili_public_metadata_when_extractor_omits_it(monkeypatch):
    payload = {
        "title": "Video title",
        "formats": [{"format_id": "1080", "height": 1080, "width": 1920, "ext": "mp4", "vcodec": "avc1", "acodec": "aac", "url": "https://example.com/video.mp4"}],
    }
