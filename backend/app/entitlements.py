...
def remaining_free_downloads(database: Database, user_id: str) -> int | None:
    ...
    row = database.fetchone(
        "SELECT count FROM daily_usage WHERE user_id = ? AND usage_date = ? AND action = 'download'",
        (user_id, date.today().isoformat()),
    )
    return max(0, settings.free_daily_downloads - int(row["count"] if row else 0))
    ...
    try:
        with database.transaction(immediate=True) as connection:
            existing = connection.execute(
                "SELECT 1 FROM action_requests WHERE user_id = ? AND action = 'download' AND request_key = ?",
                (user_id, request_key),
            ).fetchone()
            ...
            if existing:
                return max(0, settings.free_daily_downloads - count)
            if count >= settings.free_daily_downloads:
                raise HTTPException(429, "今日 5 次免费下载额度已用完，开通 VIP 可无限下载")
            connection.execute(
                "INSERT INTO action_requests(user_id, action, request_key, created_at) VALUES (?, 'download', ?, ?)",
                (user_id, request_key, utcnow_iso()),
            )
            connection.execute(
                """INSERT INTO daily_usage(user_id, usage_date, action, count) VALUES (?, ?, 'download', 1)
                   ON CONFLICT(user_id, usage_date, action) DO UPDATE SET count = count + 1""",
                (user_id, today),
            )
            return max(0, settings.free_daily_downloads - count - 1)
    except sqlite3.IntegrityError:
        return remaining_free_downloads(database, user_id)
...
def consume_anonymous_download(database: Database, client_address: str, request_key: str) -> int:
    ...
    with database.transaction(immediate=True) as connection:
        ...
        if existing:
            return max(0, settings.free_daily_downloads - count)
        if count >= settings.free_daily_downloads:
            raise HTTPException(429, "今日 5 次免费下载额度已用完，登录并开通 VIP 可无限下载")
        connection.execute(
